		...
		//暂时设计每个slide大小需要一致
		  ...
		  var pageSwiper = new Swiper('#page', {
		  	...
		  	on: {
		  		touchMove: function() {
		  			...
		  			for (i = 0; i < this.slides.length; i++) {
		  				...
		  				if (Math.abs(slideProgress) < 1) {
		  					navSwiper.slides.eq(i).find('span').css('color', 'rgba(25,79,144,1)')
		  				}
		  			}
		  		},
		  		transitionStart: function() {
		  			...
		  			navSwiper.slides.eq(activeIndex).find('span').css('color', 'rgba(25,79,144,1)')
		  			if (activeIndex > 0) {
		  ...
		  navSwiper.$el.on('touchstart', function(e) {
		   ...
		   navSwiper.on('tap', function(e) {
		  	...
		  	clickSlide.find('span').css('color', 'rgba(25,79,144,1)');
		  })
		  //内容滚动			
		  var scrollSwiper = new Swiper('.scroll', {
		  	...
		  	on: {
		  		touchMove: function() {
		  			...
		  			if (this.translate > 43 - 36 && this.translate < 43) {
		  				topBar.transform('translateY(' + (this.translate - 43) + 'px)');
		  			}
		  		...
		  		touchEnd: function() {
		  			...
		  			if (this.translate > 36 && this.translate < 43 && this.translate < startPosition) {
		  			...
		  			if (this.translate > 36 && this.translate < 43 && this.translate > startPosition) {
		  				...
		  				for (sc = 0; sc < scrollSwiper.length; sc++) {
		  					if (scrollSwiper[sc].translate < 43 && scrollSwiper[sc].translate > 0) {
		  						scrollSwiper[sc].setTransition(tSpeed);
		  						scrollSwiper[sc].setTranslate(43)
		  					}
		  				}
